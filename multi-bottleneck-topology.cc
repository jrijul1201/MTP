...
{
  std::string query = "/NodeList/" + std::to_string (queue_number) +
...
TraceBottleneckTx (uint32_t pktSize)
{
  ...
  *bottleneckTransmittedStream->GetStream ()
      << Simulator::Now ().GetSeconds () << "\t" << btl_thr << std::endl;
...
main (int argc, char *argv[])
{
  ...
  std::string RTT = "192ms"; //round-trip time of each TCP flow
  CommandLine cmd (__FILE__);
  cmd.AddValue ("qTh", "Qth", qTh);
  cmd.AddValue ("thEnabled", "Is threshold Enabled", thEnabled);
  cmd.AddValue ("roundTripTime", "Is threshold Enabled", RTT);
  cmd.Parse (argc, argv);
  dir += (thEnabled ? std::to_string (qTh) + "-" + RTT : "-" + RTT) + "/";
  ...
  for (uint32_t i = 0; i < number_of_links; i++)
    {
      ...
      switch (i)
        ...
        case 71:
          bottleneck_p2p.SetDeviceAttribute ("DataRate", StringValue ("80Mbps"));
          queue_size = thEnabled ? qTh : 2084 * (0.8);
          bottleneck_p2p.SetQueue ("ns3::DropTailQueue<Packet>", "MaxSize",
                                   QueueSizeValue (QueueSize (std::to_string (queue_size) + "p")));
          break;
        case 73:
  ...
  Simulator::Schedule (Seconds (stime), &TraceDroppedPacket, dir + iterator + "Loss.txt");
